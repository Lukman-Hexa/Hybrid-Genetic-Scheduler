...
function displayResults(result) {
    ...
    if (result.comparison) {
        ...
        <div class="mb-4 bg-purple-900 bg-opacity-30 border-2 border-purple-400 rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <!-- Genetika Murni -->
                <div class="bg-slate-700 bg-opacity-50 p-4 rounded ${winner === 'ga_only' ? 'border-2 border-yellow-400' : 'border border-slate-600'}">
                    <div class="text-center mb-3">
                        <div class="text-blue-300 font-bold">🧬 GA Murni</div>
                        <div class="text-gray-400 text-xs mt-1">Tournament Selection + Crossover + Mutasi</div>
                        ${winner === 'ga_only' ? '<div class="mt-1 text-yellow-400 text-xs font-bold">🏆 HASIL TERBAIK</div>' : ''}
                    </div>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            <span class="text-gray-400">Penalti Akhir:</span>
                            <span class="text-white font-mono font-bold">${comp.ga_only.penalty.toFixed(2)}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-300 font-mono">${comp.ga_only.total_sessions}</span>
                        </div>
                    </div>
                    <div class="mt-3 pt-2 border-t border-slate-600">
                        <button onclick="switchToMethodSchedule('ga_only')"
                            class="w-full text-xs py-1 px-2 bg-blue-600 hover:bg-blue-500 text-white rounded transition">
                            <i class="fas fa-calendar mr-1"></i> Lihat Jadwal GA Murni
                        </button>
                    </div>
                </div>
                
                <!-- Genetika + Greedy -->
                <div class="bg-slate-700 bg-opacity-50 p-4 rounded ${winner === 'ga_greedy' ? 'border-2 border-yellow-400' : 'border border-slate-600'}">
                    <div class="text-center mb-3">
                        <div class="text-green-300 font-bold">⚡ GA + Greedy</div>
                        <div class="text-gray-400 text-xs mt-1">Seleksi + Mutasi + Restart + Greedy Post-processing</div>
                        ${winner === 'ga_greedy' ? '<div class="mt-1 text-yellow-400 text-xs font-bold">🏆 HASIL TERBAIK</div>' : ''}
                    </div>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            <span class="text-gray-400">Penalti Akhir:</span>
                            <span class="text-white font-mono font-bold">${comp.ga_greedy.penalty.toFixed(2)}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-500">• Fase GA:</span>
                            <span class="text-gray-300 font-mono">${comp.ga_greedy.ga_time.toFixed(2)}s</span>
                        </div>
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-500">• Fase Greedy:</span>
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-green-400 font-mono">${comp.ga_greedy.improvements}x</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Reduksi Penalti:</span>
                            <span class="text-green-400 font-mono">-${comp.ga_greedy.penalty_reduction.toFixed(2)}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-400">Status:</span>
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-400">Sesi:</span>
                            <span class="text-gray-300 font-mono">${comp.ga_greedy.total_sessions}</span>
                        </div>
                    </div>
                    <div class="mt-3 pt-2 border-t border-slate-600">
                        <button onclick="switchToMethodSchedule('ga_greedy')"
                            class="w-full text-xs py-1 px-2 bg-green-700 hover:bg-green-600 text-white rounded transition">
                            <i class="fas fa-calendar mr-1"></i> Lihat Jadwal GA+Greedy
                        </button>
                    </div>
                </div>
            </div>
            ...
            <!-- Perbedaan mekanisme -->
            <div class="mb-3 bg-slate-800 bg-opacity-60 rounded p-3">
                <div class="text-xs text-gray-400 mb-1 font-semibold">📋 Perbedaan Mekanisme</div>
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div>
                        <div class="text-blue-300 font-semibold mb-1">GA Murni</div>
                        <div class="text-gray-400">✓ Tournament Selection</div>
                        <div class="text-gray-400">✓ Single-Point Crossover</div>
                        <div class="text-gray-400">✓ Adaptive Mutation</div>
                        <div class="text-gray-500">✗ Restart Mechanism</div>
                        <div class="text-gray-500">✗ Greedy Post-processing</div>
                    </div>
                    <div>
                        <div class="text-green-300 font-semibold mb-1">GA + Greedy</div>
                        <div class="text-gray-400">✓ Seleksi Elitisme</div>
                        <div class="text-gray-500">✗ Crossover (bukan mekanismenya)</div>
                        <div class="text-gray-400">✓ Adaptive Mutation</div>
                        <div class="text-gray-400">✓ Restart (anti-local optimum)</div>
                        <div class="text-gray-400">✓ Greedy Post-processing</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-800 bg-opacity-50 p-3 rounded text-center">
                <div class="text-sm text-gray-400 mb-1">Selisih Penalti (GA Murni vs GA+Greedy)</div>
                <div class="text-2xl font-bold ${penaltyImprovement > 0 ? 'text-green-400' : (penaltyImprovement < 0 ? 'text-red-400' : 'text-gray-400')}">
                    ${penaltyImprovement > 0 ? 'GA+Greedy lebih baik: -' : (penaltyImprovement < 0 ? 'GA Murni lebih baik: -' : '')}${Math.abs(penaltyImprovement).toFixed(2)}
                    ${penaltyImprovement !== 0 ? ` (${Math.abs(percentageImprovement).toFixed(1)}%)` : '(Hasil Sama)'}
                </div>
                <div class="text-xs text-gray-400 mt-1">
                    ${winner === 'ga_greedy' ? '✅ Greedy post-processing berhasil memperbaiki solusi GA' :
                      winner === 'ga_only'   ? '✅ Crossover GA murni menghasilkan solusi lebih baik' :
                                              '⚖️ Kedua metode menghasilkan kualitas setara'}
                </div>
            </div>
        </div>`;

        // Simpan data perbandingan ke window untuk akses tab
        window._comparisonData = comp;
    }
    ...
    // ⭐ Dynamic semester tabs — jika compare mode, tambahkan tabs untuk kedua jadwal
    ...
    if (tabsContainer) {
        ...
        if (result.comparison) {
            // Mode perbandingan: dua set tab jadwal
            tabsHTML += `<span class="text-gray-500 text-xs px-2 self-center">── GA Murni ──</span>`;
            semesters.forEach(sem => {
                tabsHTML += `<button onclick="switchTab('ga_only_sem${sem}')" class="tab-btn px-3 py-2 hover:bg-blue-800 text-blue-300 rounded font-semibold whitespace-nowrap text-sm transition border border-blue-700">🧬 Sem ${sem}</button>`;
            });
            tabsHTML += `<span class="text-gray-500 text-xs px-2 self-center">── GA+Greedy ──</span>`;
            semesters.forEach(sem => {
                tabsHTML += `<button onclick="switchTab('ga_greedy_sem${sem}')" class="tab-btn px-3 py-2 hover:bg-green-800 text-green-300 rounded font-semibold whitespace-nowrap text-sm transition border border-green-700">⚡ Sem ${sem}</button>`;
            });
        } else {
            semesters.forEach(sem => {
    ...
    // Render jadwal terbaik (default) di tab standar
    semesters.forEach(sem => {
    ...
    // Jika compare mode, render juga kedua jadwal di tab terpisah
    if (result.comparison) {
        const comp = result.comparison;
        
        // Buat container tab untuk GA Murni per semester
        semesters.forEach(sem => {
            createOrGetTabContent(`ga_only_sem${sem}`);
            const semData = comp.ga_only.structured_result[sem];
            if (semData) {
                renderSemesterScheduleInDiv(`ga_only_sem${sem}Content`, sem, semData, 'ga_only');
            }
        });
        
        // Buat container tab untuk GA+Greedy per semester
        semesters.forEach(sem => {
            createOrGetTabContent(`ga_greedy_sem${sem}`);
            const semData = comp.ga_greedy.structured_result[sem];
            if (semData) {
                renderSemesterScheduleInDiv(`ga_greedy_sem${sem}Content`, sem, semData, 'ga_greedy');
            }
        });
    }
}
...
/**
 * Buat atau dapatkan elemen tab-content untuk ID tertentu.
 * Tab baru dibuat di dalam resultsContainer agar switchTab bisa menemukan dan menyembunyikannya.
 */
function createOrGetTabContent(tabId) {
    let el = $(tabId);
    if (!el) {
        ...
        contentEl.id = `${tabId}Content`;
        el.appendChild(contentEl);
        
        const resultsContainer = $('resultsContainer');
        if (resultsContainer) resultsContainer.appendChild(el);
    }
    return el;
}

/**
 * Render jadwal semester ke dalam div tertentu dengan label metode.
 */
function renderSemesterScheduleInDiv(divId, semester, semesterData, method) {
    const contentDiv = $(divId);
    if (!contentDiv) return;
    
    const methodLabel = method === 'ga_only'
        ? '<span class="text-blue-300 text-xs font-semibold px-2 py-1 bg-blue-900 bg-opacity-50 rounded">🧬 GA Murni (Crossover)</span>'
        : '<span class="text-green-300 text-xs font-semibold px-2 py-1 bg-green-900 bg-opacity-50 rounded">⚡ GA + Greedy (Restart)</span>';
    
    const borderColor = method === 'ga_only' ? 'border-blue-500' : 'border-green-500';
    
    let html = `<div class="mb-3 flex items-center gap-2">
        <span class="text-gray-400 text-sm">Semester ${semester}</span>
        ${methodLabel}
    </div>`;
    ...
    dayOrder.forEach(day => {
        if (semesterData[day]) {
            ...
            semesterData[day].forEach(session => {
                html += `
                    <div class="bg-slate-700 p-3 rounded border-l-4 ${borderColor}">
...
/**
 * Shortcut tombol "Lihat Jadwal" di card perbandingan.
 * Langsung switch ke tab semester pertama dari metode yang dipilih.
 */
function switchToMethodSchedule(method) {
    const comp = window._comparisonData;
    if (!comp) return;
    
    const structured = method === 'ga_only' ? comp.ga_only.structured_result : comp.ga_greedy.structured_result;
    const semesters = Object.keys(structured).map(s => parseInt(s)).sort((a, b) => a - b);
    
    if (semesters.length > 0) {
        const tabId = `${method}_sem${semesters[0]}`;
        const tabEl = $(tabId);
        if (tabEl) {
            // Simulasi klik tab
            ...
            document.querySelectorAll('.tab-btn').forEach(btn => {
                btn.classList.remove('bg-blue-500', 'text-white', 'bg-blue-600', 'bg-green-700');
