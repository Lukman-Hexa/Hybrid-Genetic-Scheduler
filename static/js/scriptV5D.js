    ...
    Object.entries(rooms).forEach(([id, data]) => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-1">
                <button onclick="editRoom('${id}')" title="Edit"
                ...
                <button onclick="deleteRoom('${id}')" title="Hapus"
    ...
    Object.entries(dosens).forEach(([id, data]) => {
        ...
        div.innerHTML = `
            ...
            <div class="flex gap-1 flex-shrink-0 ml-2">
                <button onclick="editDosen('${id}')" title="Edit"
                    class="text-yellow-400 hover:text-yellow-300 hover:bg-yellow-900 hover:bg-opacity-30 p-2 rounded-lg transition text-sm">
                    <i class="fas fa-edit"></i>
                </button>
                <button onclick="deleteDosen('${id}')" title="Hapus"
                    class="text-red-400 hover:text-red-300 hover:bg-red-900 hover:bg-opacity-30 p-2 rounded-lg transition text-sm">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
    ...
    Object.keys(grouped).sort((a, b) => a - b).forEach(sem => {
        ...
        grouped[sem].forEach(({ name, dosen, kapasitas_kelas, jam, tipe }) => {
            ...
            div.innerHTML = `
                ...
                <div class="flex gap-1 flex-shrink-0 ml-2">
                    <button onclick="editCourse('${name.replace(/'/g, "\\'")}')" title="Edit"
                        class="text-yellow-400 hover:text-yellow-300 hover:bg-yellow-900 hover:bg-opacity-30 p-2 rounded-lg transition text-sm">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="deleteCourse('${name.replace(/'/g, "\\'")}')" title="Hapus"
                        class="text-red-400 hover:text-red-300 hover:bg-red-900 hover:bg-opacity-30 p-2 rounded-lg transition text-sm">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
...
function displayResults(result) {
    ...
    // Di mode compare, gabungkan semester dari kedua hasil
    let allSemesters = Object.keys(result.structured_result).map(s => parseInt(s));
    if (result.comparison) {
        const semGA = Object.keys((result.comparison.ga_only.structured_result) || {}).map(s => parseInt(s));
        const semGreedy = Object.keys((result.comparison.ga_greedy.structured_result) || {}).map(s => parseInt(s));
        allSemesters = [...new Set([...allSemesters, ...semGA, ...semGreedy])];
    }
    const semesters = allSemesters.sort((a, b) => a - b);
    ...
    if (result.comparison) {
        // Mode compare: tampilkan jadwal KEDUA algoritma per semester dengan toggle
        const comp = result.comparison;
        semesters.forEach(sem => {
            const semDataGA = (comp.ga_only.structured_result || {})[sem];
            const semDataGreedy = (comp.ga_greedy.structured_result || {})[sem];
            renderSemesterScheduleCompare(sem, semDataGA, semDataGreedy);
        });
    } else {
        // Single mode: tampilkan jadwal terbaik saja
        semesters.forEach(sem => {
...
function buildScheduleHTML(semesterData, borderColor) {
    if (!semesterData) return '<div class="text-gray-400 text-center p-4">Data tidak tersedia</div>';
    let html = '';
    ...
    return html || '<div class="text-gray-400 text-center p-4">Tidak ada sesi pada semester ini</div>';
}
...
function renderSemesterSchedule(semester, semesterData) {
    ...
    contentDiv.innerHTML = buildScheduleHTML(semesterData, 'border-blue-400');
}

function renderSemesterScheduleCompare(semester, semDataGA, semDataGreedy) {
    const contentDiv = $(`sem${semester}Content`);
    if (!contentDiv) {
        console.warn(`Content div for semester ${semester} not found`);
        return;
    ...
    const gaHTML = buildScheduleHTML(semDataGA, 'border-blue-400');
    const greedyHTML = buildScheduleHTML(semDataGreedy, 'border-purple-400');
    
    contentDiv.innerHTML = `
        <div class="flex gap-2 mb-4">
            <button onclick="switchCompareView('ga_sem${semester}', 'greedy_sem${semester}', this)" 
                class="compare-toggle-btn active-toggle px-4 py-1.5 rounded text-sm font-semibold bg-blue-500 text-white transition">
                🧬 Genetika Murni
            </button>
            <button onclick="switchCompareView('greedy_sem${semester}', 'ga_sem${semester}', this)"
                class="compare-toggle-btn px-4 py-1.5 rounded text-sm font-semibold bg-slate-600 text-gray-300 hover:bg-slate-500 transition">
                🔀 GA + Greedy
            </button>
        </div>
        <div id="ga_sem${semester}">${gaHTML}</div>
        <div id="greedy_sem${semester}" class="hidden">${greedyHTML}</div>
    `;
}

function switchCompareView(showId, hideId, btn) {
    $(showId).classList.remove('hidden');
    $(hideId).classList.add('hidden');
    // Update button styles in same container
    const container = btn.closest('.space-y-3, div');
    btn.closest('div').querySelectorAll('.compare-toggle-btn').forEach(b => {
        b.classList.remove('bg-blue-500', 'text-white');
        b.classList.add('bg-slate-600', 'text-gray-300');
    });
    btn.classList.remove('bg-slate-600', 'text-gray-300');
