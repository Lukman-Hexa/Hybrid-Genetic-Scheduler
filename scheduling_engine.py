...
class SchedulingContext:
    ...
    def _build_time_slots(self):
        ...
        all_slots_flat = [
            slot for day_slots in all_slots.values() for slot in day_slots
        ]
...
def get_valid_slots_for_session(context, session_id):
    ...
    for day in available_days:
        ...
        for i in range(len(slots_in_day)):
            ...
            slots_to_check = get_session_time_slots(
                context, day, start_time_str, num_slots
            )
            if slots_to_check:
...
def count_conflicts(context, schedule, new_session_id):
    ...
    new_slots = get_session_time_slots(
        context, new_day, new_start_str, new_course_data["duration_slots"]
    )
    ...
    for other_session_id, (other_room, other_time) in schedule.items():
        ...
        other_slots = get_session_time_slots(
            context, other_day, other_start_str, other_course_data["duration_slots"]
        )
        ...
        if overlap:
            ...
            if new_course_data["dosen"] == other_course_data["dosen"]:
                conflicts += 1
            if new_course_data["sem"] == other_course_data["sem"] and is_class_conflict(
                new_course_data["kelas"], other_course_data["kelas"]
            ):
                conflicts += 1
...
def create_schedule_greedy(context):
    ...
    for session_id in sessions_sorted:
        ...
        sampled_rooms = random.sample(
            list(context.rooms.keys()), min(3, len(context.rooms))
        )
        ...
        if best_slot and best_room:
            ...
            schedule[session_id] = (
                random.choice(list(context.rooms.keys())),
                random.choice(valid_slots),
            )
...
def create_schedule(context):
    ...
    for session_id in context.all_sessions:
        ...
        time = (
            random.choice(valid_slots)
            if valid_slots
            else random.choice(context.time_slots[0])
        )
...
def calculate_fitness(schedule, context):
    ...
    for session_id, (room, time_start) in schedule.items():
        ...
        slots_to_check = get_session_time_slots(
            context, day, start_time_only, num_slots
        )
    ...
    for i, session_i in enumerate(all_sessions_data):
        for j, session_j in enumerate(all_sessions_data):
            ...
            if session_i["day"] == session_j["day"] and time_overlap(
                session_i["start"],
                session_i["end"],
                session_j["start"],
                session_j["end"],
                ...
                if session_i["semester"] == session_j["semester"] and is_class_conflict(
                    session_i["kelas"], session_j["kelas"]
                ):
                    penalty += context.ga_params["HARD_CONSTRAINT_PENALTY"]

    # Check dosen conflicts ACROSS semesters
    dosen_conflicts, conflict_details = check_dosen_conflict_across_semesters(
        all_sessions_data
    )
    ...
    for session in all_sessions_data:
        ...
        kelas = session["kelas"]
        base_course = (
            "_".join(course_full_id.split("_")[:-1])
            if "_" in course_full_id
            else course_full_id
        )
                ...
                for j in range(i + 1, len(sessions_list)):
                    if sessions_list[i]["day"] == sessions_list[j]["day"]:
                        if sessions_list[i]["end"] <= sessions_list[j]["start"]:
                            gap_hours = (
                                sessions_list[j]["start"] - sessions_list[i]["end"]
                            ).total_seconds() / 3600
                        elif sessions_list[j]["end"] <= sessions_list[i]["start"]:
                            gap_hours = (
                                sessions_list[i]["start"] - sessions_list[j]["end"]
                            ).total_seconds() / 3600
                        else:
                            gap_hours = 0
    ...
    utilization, util_penalty = calculate_room_utilization(
        schedule, context.rooms, context
    )
...
def mutate(schedule, context, mutation_rate):
    ...
    if random.random() < mutation_rate:
        num_mutations = random.randint(
            1, min(5, max(1, len(context.all_sessions) // 4))
        )
        sessions_to_mutate = random.sample(
            context.all_sessions, min(num_mutations, len(context.all_sessions))
        )
    ...
    try:
        ...
        for iteration in range(max_iterations):
            ...
            for session_id in context.all_sessions:
                ...
                valid_slots = get_valid_slots_for_session(context, session_id)
                slots_to_check = (
                    valid_slots[:20] if len(valid_slots) > 20 else valid_slots
                )
...
def run_ga_pure(context, ga_params, progress_callback=None, seed=None):
    ...
    for generation in range(ga_params["MAX_GENERATIONS"]):
        ...
        mutation_rate = ga_params["INITIAL_MUTATION_RATE"] - (
            (ga_params["INITIAL_MUTATION_RATE"] - ga_params["MIN_MUTATION_RATE"])
            * progress
        )
        ...
        # Catat durasi generasi ini untuk moving average ETA
        now = datetime.now()
        ...
        if generation % LOG_INTERVAL == 0:
            ...
            eta_min = min(eta_sec, eta_sec_overall)
            eta_max = max(eta_sec, eta_sec_overall)

            improvement_marker = ""
            if best_overall["fitness"] < last_logged_fitness:
                improvement_marker = (
                    f" ↓ (-{last_logged_fitness - best_overall['fitness']:.2f})"
                )
            elif stagnant_gens >= LOG_INTERVAL:
                improvement_marker = f" ⚠ stagnan {stagnant_gens} gen"

            print(
                f"[{progress * 100:5.1f}%] Gen {generation:>5} | "
                f"Penalti = {best_overall['fitness']:>8.2f}{improvement_marker} | "
                f"MutRate = {mutation_rate:.3f} | "
                f"ETA ≈ {eta_min / 60:.1f}–{eta_max / 60:.1f} mnt"
            )
            last_logged_fitness = best_overall["fitness"]
                ...
                progress_callback(
                    {
                        ...
                        "penalty": best_overall["fitness"],
                        "mutation_rate": round(mutation_rate, 4),
                        "stagnant": stagnant_gens,
        ...
        new_pop = [
            {
                "schedule": {k: v for k, v in ind["schedule"].items()},
                "fitness": ind["fitness"],
            }
    ...
    print(
        f"GA Murni selesai: Penalti = {best_overall['fitness']:.2f} | Waktu = {ga_duration:.2f}s | Generasi = {generation + 1}"
    )
...
def run_ga_greedy(context, ga_params, progress_callback=None, seed=None):
    ...
    for generation in range(ga_params["MAX_GENERATIONS"]):
        ...
        if generation % LOG_INTERVAL == 0:
            ...
            elif generations_without_improvement >= LOG_INTERVAL:
                improvement_marker = f" ⚠ stagnan {generations_without_improvement} gen"

            print(
                f"[{progress * 100:5.1f}%] Gen {generation:>5} | "
                f"Penalti = {best_overall['fitness']:>8.2f}{improvement_marker} | "
                f"MutRate = {mutation_rate:.3f} | "
                f"Stagnan = {generations_without_improvement}/{stagnation_threshold} | "
                f"ETA ≈ {eta_min / 60:.1f}–{eta_max / 60:.1f} mnt"
            )
            last_logged_fitness = best_overall["fitness"]
                ...
                progress_callback(
                    {
                        ...
                        "penalty": best_overall["fitness"],
                        "mutation_rate": round(mutation_rate, 4),
                        "stagnant": generations_without_improvement,
        ...
        if generations_without_improvement > stagnation_threshold:
            print(
                f"Restart: Stagnan {stagnation_threshold} generasi | Best: {best_overall['fitness']:.2f}"
            )
            ...
            for _ in range(ga_params["POPULATION_SIZE"] - elite_keep):
                ...
                new_population.append(
                    {"schedule": new_schedule, "fitness": new_fitness}
                )
        ...
        while len(new_pop) < ga_params["POPULATION_SIZE"]:
            ...
            new_pop.append(
                {"schedule": offspring_schedule, "fitness": offspring_fitness}
            )
    ...
    print(
        f"GA (fase 1) selesai: Penalti = {best_overall['fitness']:.2f} | Waktu = {ga_duration:.2f}s"
    )
    ...
    print(
        f"Waktu greedy: {greedy_duration:.2f}s | Total: {ga_duration + greedy_duration:.2f}s"
    )
