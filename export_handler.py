...
def export_to_excel(final_schedule, structured_result, filepath, semester_type='ganjil'):
    wb = Workbook()
    wb.remove(wb.active)
...
def setup_summary_sheet(ws, final_schedule, structured_result, semester_type):
    ...
    for sem in sorted(structured_result.keys()):
        total_sessions = sum(len(sessions) for sessions in structured_result[sem].values())
...
def setup_semester_sheet(ws, semester_data, semester_num):
    ...
    headers = ["Hari", "Jam Mulai", "Jam Selesai", "Mata Kuliah", "Kelas", "Dosen", "Ruangan"]
